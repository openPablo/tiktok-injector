...
class TikTokUploader(baseUrl: String, pathToDriver: String, chromeProfile: String) :
    ...
    fun upload(video_path: String, tags: String) {
        ...
        sendTextByDivXpath(
            tags,
            "//html/body/div[1]/div/div/div/div/div[2]/div[2]/div[1]/div/div[1]/div[2]/div/div[1]/div/div/div/div/div/div"
        )
        ...
        sleep(3000, 6000)
        clickByXpath("/html/body/div[3]/div/div/div[1]/div[2]")
    }

    private fun selectTiktokIframe() {
        if (!iframeSelected) {
            val timout = Duration.ofSeconds(10)
            val iframe = WebDriverWait(
                driver,
                timout
            ).until(ExpectedConditions.presenceOfElementLocated(By.xpath("//html/body/div/div/div[2]/div/iframe")))
